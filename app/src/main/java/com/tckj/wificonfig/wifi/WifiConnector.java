package com.tckj.wificonfig.wifi;
...
import com.tckj.wificonfig.wifi.interfaces.RemoveWifiListener;
import com.tckj.wificonfig.wifi.interfaces.ShowWifiListener;
import com.tckj.wificonfig.wifi.interfaces.WifiStateListener;
...
public class WifiConnector {
     ...
     */
    protected RemoveWifiListener removeWifiListener;
    ...
    public WifiConnector(Context context, String SSID, String password) {
        this.context = context;
        this.wifiManager = (WifiManager) context.getApplicationContext().getSystemService(Context.WIFI_SERVICE);
        String[] type = getBssidAndCiperType(SSID);
        if (type == null || type.length != 2) {
            setWifiConfiguration(SSID, "", SECURITY_NONE, password);
        } else {
            setWifiConfiguration(SSID, type[0], type[1], password);
        }
    ...
    private String[] getBssidAndCiperType(String ssid) {
        List<ScanResult> list = wifiManager.getScanResults();
        for (ScanResult scResult : list) {
            if (!TextUtils.isEmpty(scResult.SSID) && scResult.SSID.equals(ssid)) {
                String capabilities = scResult.capabilities;
                String bssid = scResult.BSSID;
                if (!TextUtils.isEmpty(capabilities)) {
                    if (capabilities.contains("WPA")
                            || capabilities.contains("wpa")) {
                        return new String[]{bssid, SECURITY_WPA};
                    } else if (capabilities.contains("WEP")
                            || capabilities.contains("wep")) {
                        return new String[]{bssid, SECURITY_WEP};
                    } else {
                        return new String[]{bssid, SECURITY_NONE};
                    }
                }
            }
        }
